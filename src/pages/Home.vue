...
const getHomeSliderImages = () => {
   return (
      appStore.categories.find((c) => c.category_name_ascii === "home")
         ?.category_slider.slider.slider_images || []
   );
};
...
<template>
   <template v-if="appStore.status === 'loading'">
   ...
   <Slider v-else :quantity="2" :sliderImages="sliderImages" :key="sliderImages.length" />
   <div class="mt-[30px]">
      <h2 class="text-[18px] md:text-[22px] font-[500] text-[#333]">
         New Products
      </h2>
