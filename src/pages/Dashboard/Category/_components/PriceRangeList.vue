...
const handleAddBrand = async (data: PriceRangeSchema, type: "add" | "edit") => {
   ...
   switch (type) {
      ...
      case "edit":
         if (!curPriceRange.value || curPriceRangeIndex.value === undefined) return;
...
<template>
   <h1 :class="mainClasses.label">Price ranges</h1>
   <div v-if="!!categories.length" :class="mainClasses.group">
      ...
      <template v-if="!!curCategory">
         <div
            :class="`flex flex-wrap  -mt-[8px] space-x-[8px] ${
               isFetching ? 'disable' : ''
            }`"
         >
            <template v-for="(priceRange, index) in priceRangesByCategory">
               <div :class="classes.priceRangeItem">
                  ...
                  <div
                     class="flex items-center space-x-[6px] ml-[10px] pl-[10px] border-l border-black/15"
                  >
                     <button
                        :class="classes.cta"
                        :onClick="
