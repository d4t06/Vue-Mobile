import type { ImageType, SliderImage, SliderImageSchema } from "@/types";
import { ref } from "vue";
      ...
      try {
         switch (props.type) {
            case "add-image":
               ...
               const res = await publicRequest.post(SLIDER_IMAGE_URL, props.sliderImage);
               const newSliderImage = res.data.data as SliderImage;
               newSliderImage.image = props.image 

               appStore.categories[props.sliderIndex].category_slider.slider.slider_images.push(
                  newSliderImage
               );
            ...
            case "delete-image":
               ...
               await publicRequest.delete(`${SLIDER_IMAGE_URL}/${props.sliderImageID}`);

               appStore.categories[props.sliderIndex].category_slider.slider.slider_images.splice(
                  props.sliderIndex,
                  1
               );
